...
def parse_args():
    ...
    parser.add_argument(
        "--lang",
        help="language",
        choices=["ar", "de", "en", "es", "es_en", "fr", "ger", "hi", "hy", "mr", "pt", "ru", "sv", "vi", "zh", 'ja'],
        default="en",
        type=str,
    )
...
if __name__ == "__main__":
    ...
    if sum(token_count_per_type.values()) == 0:
        print("- No tokens found for evaluation. Check your input file and filter criteria.")
    else:
        print("- Accuracy: " + str(sum(token_weighted_accuracy) / sum(token_count_per_type.values())))
